...
class WoManagementInfoCallPopup extends Component {
    constructor(props) {
        super(props);
    ...
    buildTableColumns() {
        return [
            {
                Header: <Trans i18nKey="woManagement:woManagement.label.userCall" />,
                id: "userCall",
                accessor: d => <span title={d.userCall}>{d.userCall}</span>
            },
            {
                Header: <Trans i18nKey="woManagement:woManagement.label.startCallTime" />,
                id: "startCallTime",
                accessor: d => <span title={d.startCallTime}>{d.startCallTime}</span>
            },
            {
                Header: <Trans i18nKey="woManagement:woManagement.label.receiveUnitInfoCall" />,
                id: "userName",
                accessor: d => <span title={d.userName}>{d.userName}</span>
            },
            {
                Header: <Trans i18nKey="woManagement:woManagement.label.phone" />,
                id: "phone",
                accessor: d => <span title={d.phone}>{d.phone}</span>,
            },
            {
                Header: <Trans i18nKey="woManagement:woManagement.label.resultInfoCall" />,
                id: "result",
                accessor: d => <span title={d.result}>{d.result}</span>,
            },
            {
                Header: <Trans i18nKey="woManagement:woManagement.label.resultTime" />,
                id: "resultTime",
                accessor: d => <span title={d.resultTime}>{d.resultTime}</span>,
            },
            {
                Header: <Trans i18nKey="woManagement:woManagement.label.description" />,
                id: "description",
    ...
    viewCall = () => {
        this.props.actions.viewCall(this.state.objectSearch).then((response) => {
            ...
            toastr.error(this.props.t("woManagement:woManagement.message.error.getViewCall"));
        });
    }
        ...
        return (
            <Modal isOpen={this.props.parentState.isOpenInfoCallPopup} className={'modal-primary modal-lg ' + this.props.className}
                ...
                <ModalHeader toggle={this.closePopup}>
                    {t("woManagement:woManagement.button.infoCall")}
                </ModalHeader>
                <ModalBody>
                    <Row>
                        <Col xs="12">
...
WoManagementInfoCallPopup.propTypes = {
...
export default connect(mapStateToProps, mapDispatchToProps)(translate()(WoManagementInfoCallPopup));
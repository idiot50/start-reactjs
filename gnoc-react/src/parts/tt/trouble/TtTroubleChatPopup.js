...
class TtTroubleChatPopup extends Component {
    constructor(props) {
        super(props);
    ...
    buildTableColumns() {
        return [
            {
                Header: <Trans i18nKey="ttTrouble:ttTrouble.label.username" />,
                id: "username",
                accessor: d => <span title={d.username}>{d.username}</span>
            },
            {
                Header: <Trans i18nKey="ttTrouble:ttTrouble.label.fullname" />,
                id: "fullname",
            ...
            {
                Header: <Trans i18nKey="ttTrouble:ttTrouble.label.email" />,
                id: "email",
                accessor: d => <span title={d.email}>{d.email}</span>,
            },
            {
                Header: <Trans i18nKey="ttTrouble:ttTrouble.label.phone" />,
                id: "mobile",
    ...
    updateChat = () => {
        this.setState({
            ...
            if (this.state.data2.length < 1) {
                toastr.warning(this.props.t("ttTrouble:ttTrouble.message.required.users"));
                this.setState({
                    btnAddOrEditLoading: false
                });
                return;
    ...
    getListUserSearch(isSearchClicked = false) {
        this.props.actions.getListUsers(this.state.objectSearch).then((response) => {
            ...
            toastr.error(this.props.t("ttTrouble:ttTrouble.message.error.searchUsers"));
        });
    }
    ...
    getListUserSearchSupport(isSearchClicked = false) {
        this.props.actions.getListUsersSupport(this.state.objectSearch).then((response) => {
            ...
            toastr.error(this.props.t("ttTrouble:ttTrouble.message.error.searchSupport"));
        });
    }
        ...
        return (
            <Modal isOpen={this.props.parentState.isOpenPopup} className={'modal-primary modal-lg ' + this.props.className}
                ...
                <ModalHeader toggle={this.closePopup}>
                    {t("ttTrouble:ttTrouble.label.addNewChat")}
                </ModalHeader>
                <ModalBody>
                                ...
                                <Row>
                                    <Col xs="12" sm="4">
                                        <CustomAvField
                                            ...
                                            label={t("ttTrouble:ttTrouble.label.username")}
                                            placeholder={t("ttTrouble:ttTrouble.placeholder.username")}
                                        />
                                    </Col>
                                    <Col xs="12" sm="4">
                                        <CustomAvField
                                            name={"objectSearch.fullname"}
                                            label={t("ttTrouble:ttTrouble.label.employeeName")}
                                            placeholder={t("ttTrouble:ttTrouble.placeholder.employeeName")}
                                        />
                                    </Col>
                                    <Col xs="12" sm="4">
                                        <Row className="mb-2">
                                        ...
                                        <Row>
                                            ...
                                            <LaddaButton type="button"
                                                ...
                                                <i className="fa fa-file"></i> <Trans i18nKey="ttTrouble:ttTrouble.button.loadGroupSupport" />
                                            </LaddaButton>
                                        </Row>
                                    </Col>
                                </Row>
                ...
                <ModalFooter>
                    <Button type="button" color="primary" disabled={this.state.data2.length < 1} onClick={() => this.updateChat()} className="ml-auto"><i className="fa fa-check"></i> {t("ttTrouble:ttTrouble.button.choose")}</Button>
...
export default connect(mapStateToProps, mapDispatchToProps)(translate()(TtTroubleChatPopup));